...
public class lvl18 : MonoBehaviour {
	...
	void Start () {
		...
		levelcomplete.text = "";
		deathcount = PlayerPrefs.GetInt ("Deaths17");
	...
	void Update()
	{
		...
		if (clock < 0) {
			...
			clock = 10;
			foreach (GameObject block in goneblocks) {
	...
	void savedeathcount()
	{
		PlayerPrefs.SetInt ("Deaths18", deathcount);
	}
